...
import type { AcaoLogicaComponivel, AtributoCompletaDto, CapacidadeInataDto, LogicaComponivel, ModificadorLogicaComponivel, PropriedadesModificadorLogicaComponivel } from 'types-nora-api';
...
import { obtemCapacidadesInatas, obtemLogicaHabilidade, salvarLogicaHabilidade } from 'Uteis/ApiConsumer/HabilidadesMiddleware';
...
type FormularioAcaoLogica = {
    nome: string;
    ordem: string;
    idCapacidadeInata: string;
};

type CampoTextoFormularioAcaoLogica = 'nome' | 'ordem' | 'idCapacidadeInata';
...
interface Contexto__PaginaModeradorConfiguracaoHabilidades__Configuracao__Props {
    ...
    capacidadesInatas: CapacidadeInataDto[];
    ...
    alteraAcaoLogica: (indice: number, campo: CampoTextoFormularioAcaoLogica, valor: string) => void;
...
export const Contexto__PaginaModeradorConfiguracaoHabilidades__Configuracao__Provider = ({ habilidade, deselecionaHabilidade }: { habilidade: Contexto__PaginaModeradorConfiguracaoHabilidades__Configuracao__Props['habilidade']; deselecionaHabilidade: Contexto__PaginaModeradorConfiguracaoHabilidades__Props['deselecionaHabilidade']; }) => {
    ...
    const [capacidadesInatas, setCapacidadesInatas] = useState<CapacidadeInataDto[]>([]);
    ...
    const podeSalvarLogica = logicaHabilidade !== null && !logicaCarregando && !salvandoLogica && acoesLogica.every(acao => acao.nome.trim().length > 0 && ehOrdemAcaoValida(acao.ordem) && ehIdCapacidadeInataValido(acao.idCapacidadeInata));
    ...
    useEffect(() => {
        ...
        async function carregaLogica(): Promise<void> {
            ...
            try {
                const [logica, capacidadesInatasCarregadas] = await Promise.all([obtemLogicaHabilidade(habilidade.id), obtemCapacidadesInatas()]);
                if (!ativo) return;

                setLogicaHabilidade(logica);
                setAcoesLogica(logica.acoes.map(acao => criaFormularioAcaoLogica(acao)));
                setCapacidadesInatas(capacidadesInatasCarregadas);
            } catch (error) {
                if (!ativo) return;

                setLogicaHabilidade(null);
                setAcoesLogica([]);
                setCapacidadesInatas([]);
    ...
    function adicionaAcaoLogica(): void {
        setAcoesLogica(acoes => [...acoes, { nome: '', ordem: obtemProximaOrdemAcao(acoes), idCapacidadeInata: '' }]);
    };

    function alteraAcaoLogica(indice: number, campo: CampoTextoFormularioAcaoLogica, valor: string): void {
    ...
    async function salvarLogica(): Promise<void> {
        ...
        if (acoes === null) {
            await toast.erro('Falha ao salvar lógica', 'Todas as ações precisam de nome, ordem inteira e domínio de interação.');
            return;
        };
...
function criaFormularioAcaoLogica(acao: AcaoLogicaComponivel): FormularioAcaoLogica {
    return { nome: acao.nome, ordem: acao.ordem.toString(), idCapacidadeInata: acao.idCapacidadeInata.toString() };
};
...
function ehIdCapacidadeInataValido(idCapacidadeInataInformado: string): boolean {
    const idCapacidadeInata = Number(idCapacidadeInataInformado);
    return idCapacidadeInataInformado.trim().length > 0 && Number.isInteger(idCapacidadeInata) && idCapacidadeInata > 0;
};
...
function montaAcoesLogica(acoes: FormularioAcaoLogica[]): AcaoLogicaComponivel[] | null {
    ...
    for (const acao of acoes) {
        ...
        const idCapacidadeInata = Number(acao.idCapacidadeInata);
        const nome = acao.nome.trim();

        if (nome.length < 1 || !Number.isInteger(ordem) || !Number.isInteger(idCapacidadeInata) || idCapacidadeInata < 1) return null;

        acoesMontadas.push({ nome, ordem, idCapacidadeInata, parametros: [] });
    };
