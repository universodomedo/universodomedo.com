...
import { aplicaZoomMouseEditor3D, finalizaArrasteCameraEditor3D, finalizaArrastePorPerdaPointerLockEditor3D, iniciaArrasteCameraEditor3D, moveMouseEditor3D } from './editor3D.eventos.mouse';
...
export function registraEventosRenderizadorEditor3D(controle: ControleEventosEditor3D): () => void {
    ...
    const aplicaZoom = (event: WheelEvent): void => aplicaZoomMouseEditor3D(controle, event);
    ...
    const bloqueiaMenuModo = (event: MouseEvent): void => {
        if (controle.refs.estado.current.modoAtual.tipo !== 'NENHUM') event.preventDefault();
    };
    ...
    controle.canvas.addEventListener('wheel', aplicaZoom, { passive: false });
