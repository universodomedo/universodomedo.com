...
export default function PaginaEditavel_Inicial() {
    ...
    return (
        <div id={styles.recipiente_conteudo_pagina_personagem_selecionado}>
            <BarraPersonagem />
            <BotaoEvoluir />
            {personagemSelecionado?.fichaVigente && personagemSelecionado?.fichaVigente.ficha.fichaDeJogo && (
                <div id={styles.recipiente_abas_fichas}>
                    <h3>Ficha Atual</h3>
                    <div id={styles.recipiente_provisorio_atributos}>
                        {personagemSelecionado?.fichaVigente.ficha.fichaDeJogo.atributos.map(atributoFicha => (
                            <p key={atributoFicha.atributo.id}>{atributoFicha.atributo.nomeAbreviado}: {atributoFicha.valor}</p>
                        ))}
                    </div>
                    <div id={styles.recipiente_provisorio_atributos}>
                        {personagemSelecionado?.fichaVigente.ficha.fichaDeJogo.pericias.sort((a, b) => a.pericia.nome.localeCompare(b.pericia.nome)).map(periciaFicha => (
                    ...
                    <div id={styles.recipiente_provisorio_atributos}>
                        {personagemSelecionado?.fichaVigente.ficha.fichaDeJogo.estatisticasDanificaveis.map(estatisticasDanificaveisFicha => (
