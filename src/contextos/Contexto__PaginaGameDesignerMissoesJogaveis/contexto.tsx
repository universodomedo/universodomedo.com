...
import { EstruturaMissoesJogaveis, EventosApiRest, type CatalogoMissaoJogavelResumo, type MissaoJogavelResumo, type PAYLOAD__CriarCatalogoMissaoJogavel, type PAYLOAD__CriarMissaoJogavel, type PAYLOAD__SalvarCatalogoMissaoJogavel, type PAYLOAD__SalvarMissaoJogavel } from 'types-nora-api';
...
export const Contexto__PaginaGameDesignerMissoesJogaveis__Provider = ({ children }: { children: ReactNode; }) => {
    ...
    const abrirCriacaoCatalogo = useCallback(() => { setEditorCatalogo({ id: null, nome: '' }); setEditorMissao(null); }, []);
    const abrirEdicaoCatalogo = useCallback((catalogo: CatalogoMissaoJogavelResumo) => { setEditorCatalogo({ id: catalogo.id, nome: catalogo.nome }); setEditorMissao(null); }, []);
    ...
    const abrirCriacaoMissao = useCallback((catalogo: CatalogoMissaoJogavelResumo) => { setEditorMissao({ id: null, fkCatalogosMissaoId: catalogo.id, nome: '', descricao: '' }); setEditorCatalogo(null); setIdsCatalogosAbertos(idsAtuais => idsAtuais.includes(catalogo.id) ? idsAtuais : [...idsAtuais, catalogo.id]); }, []);
    const abrirEdicaoMissao = useCallback((catalogo: CatalogoMissaoJogavelResumo, missao: MissaoJogavelResumo) => { setEditorMissao({ id: missao.id, fkCatalogosMissaoId: catalogo.id, nome: missao.nome, descricao: missao.descricao }); setEditorCatalogo(null); }, []);
