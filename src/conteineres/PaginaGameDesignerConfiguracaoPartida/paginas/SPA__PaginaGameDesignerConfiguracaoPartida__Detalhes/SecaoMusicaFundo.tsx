...
// Seção auto-contida da Música de Fundo da Partida: recebe a música atual já carregada (PartidaResumo), abre o seletor (Componente_Selecionador__MusicaDeFundo) e salva direto ao confirmar (não tem o que ajustar, ≠ Arte de Capa).
export function SecaoMusicaFundo({ idPartida, idMusicaInicial }: { idPartida: number; idMusicaInicial: number | null }) {
    const [idMusica, setIdMusica] = useState<number | null>(idMusicaInicial);
            ...
            {selecionando ? (
                <>
                    ...
                    <div className={styles.linha_acoes}>
                        <button type="button" className={styles.botao_acao} onClick={() => setSelecionando(false)} disabled={salvando}>Cancelar</button>
                    </div>
                </>
            ) : (
                <>
                    ...
                    <div className={styles.linha_acoes}>
                        <button type="button" className={styles.botao_acao} onClick={() => setSelecionando(true)} disabled={salvando}>{idMusica === null ? 'Escolher Música de Fundo' : 'Trocar Música de Fundo'}</button>
                        {idMusica !== null && <button type="button" className={styles.botao_remover} onClick={() => void salva(null, null)} disabled={salvando}>Remover</button>}
                    </div>
                </>
            )}
        </fieldset>
