...
export function CaminhoMovimentacaoR3F({ origem, destino, largura, altura }: CaminhoMovimentacaoR3FProps) {
    // Z-up: altura no eixo Z (o traço/anel ficam rente ao chão z≈0); torus/circle já nascem no plano XY (sem rotação).
    const origemMundo: [number, number, number] = [mundoX(origem.x, largura), mundoY(origem.y, altura), 0.06];
    const destinoMundo: [number, number, number] = [mundoX(destino.x, largura), mundoY(destino.y, altura), 0.06];

    return (
        <>
            <Line points={[origemMundo, destinoMundo]} color={COR_MOVIMENTACAO} lineWidth={2.5} dashed dashSize={0.35} gapSize={0.22} />
            <group position={[destinoMundo[0], destinoMundo[1], 0.03]}>
                <mesh>
