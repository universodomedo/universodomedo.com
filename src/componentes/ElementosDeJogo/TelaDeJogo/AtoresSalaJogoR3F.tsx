...
export function OcupanteR3F({ ocupante, largura, altura, selecionado, aoSelecionar }: OcupanteR3FProps) {
    ...
    const z = mundoZ(ocupante.posicao.y, altura);
    const alturaApoio = useAlturaApoioNoMapa(x, z);
    const aoClicar = aoSelecionar === undefined ? undefined : (e: ThreeEvent<MouseEvent>) => { e.stopPropagation(); aoSelecionar(ocupante.keySer); };
    return <FiguraSerR3F position={[x, alturaApoio, z]} corPrimaria="#2f6f86" corPele="#e0b48f" selecionado={selecionado} aoClicar={aoClicar} />;
};
...
export function InteragivelR3F({ interagivel, novo, selecionado, largura, altura, aoSelecionar }: InteragivelR3FProps) {
    ...
    const z = mundoZ(interagivel.posicao?.y ?? 0, altura);
    const alturaApoio = useAlturaApoioNoMapa(x, z);
    ...
    if (interagivel.tipo === 'ser') return <FiguraSerR3F position={[x, alturaApoio, z]} corPrimaria={novo ? '#c79a3f' : '#7484b4'} corPele="#d8b48c" selecionado={selecionado} aoClicar={aoClicar} />;
    ...
    const baseY = corpoNoMapa ? Math.max(0, alturaApoio - dims.altura) : alturaApoio;

    return (
        <group position={[x, baseY, z]} onClick={aoClicar}>
