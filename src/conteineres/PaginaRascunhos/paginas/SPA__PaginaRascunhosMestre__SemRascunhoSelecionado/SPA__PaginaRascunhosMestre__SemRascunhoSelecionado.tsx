import styles from './styles.module.css';

import { useState } from 'react';

import { useContextoPaginaRascunhosMestre__SemRascunhoSelecionado } from 'Contextos/ContextoPaginaRascunhosMestre__SemRascunhoSelecionado/contexto';
import { DivClicavel } from 'Componentes/Elementos/DivClicavel/DivClicavel';
import { ContextoCriaRascunhoProvider } from 'Contextos/ContextoCriaRascunho/contexto';
import RascunhoEmVisualizacao from 'Componentes/ElementosVisuais/ElementosIndividuaisEmListaDeVisualizacao/RascunhoEmVisualizacao/page';

export default function SPA__PaginaRascunhosMestre__SemRascunhoSelecionado() {
    const { estilosSessaoMestrada, rascunhos } = useContextoPaginaRascunhosMestre__SemRascunhoSelecionado();
    const [isModalOpen, setIsModalOpen] = useState(false);
    
    return (
        <div className={styles.recipiente_area_rascunhos}>
            <DivClicavel className={styles.botao_novo_rascunho} onClick={() => setIsModalOpen(true)}>
                <h3>Novo Rascunho</h3>
            </DivClicavel>

            <ContextoCriaRascunhoProvider estilosSessaoMestrada={estilosSessaoMestrada} isModalOpen={isModalOpen} setIsModalOpen={setIsModalOpen} />

            <div className={styles.recipiente_rascunhos}>
                {rascunhos.length > 0 ? (
                    rascunhos.map(rascunho => <RascunhoEmVisualizacao key={rascunho.id} rascunho={rascunho} />)
                ) : (
                    <h3>Nenhum Rascunho encontrado</h3>
                )}
            </div>
        </div>
    );
};