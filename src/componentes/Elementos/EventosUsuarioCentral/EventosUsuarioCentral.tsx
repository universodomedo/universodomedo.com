...
export default function EventosUsuarioCentral() {
    ...
    const { itensPendencias, itensAjudaTutoriais, carregando, aberto, listar, marcarLido, solicitarAberturaTutorial } = useContextoEventosUsuario();
    ...
    return (
        <div className={styles.central}>
            <div className={styles.painel}>
                ...
                {itensPendencias.map(item => <CartaoCentral key={item.chave} item={item} marcarLido={marcarLido} solicitarAberturaTutorial={solicitarAberturaTutorial} />)}
                ...
                {itensAjudaTutoriais.map(item => <CartaoCentral key={item.chave} item={item} marcarLido={marcarLido} solicitarAberturaTutorial={solicitarAberturaTutorial} />)}
            </div>
        </div>
    );
};

function CartaoCentral({ item, marcarLido, solicitarAberturaTutorial }: { item: CentralItem; marcarLido: (idEvento: number) => void; solicitarAberturaTutorial: (idUsuarioTutorial: number) => void }) {
