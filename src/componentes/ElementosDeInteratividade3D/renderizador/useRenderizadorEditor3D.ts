...
import { renderizacaoEditor3DExibeAmbienteEdicao, renderizacaoEditor3DPermiteControleCamera, type ModoRenderizacaoEditor3D } from './editor3D.renderizador.modo';
import { registraEventosCameraRenderizadorEditor3D } from './eventos/editor3D.eventos.camera';
...
export function useRenderizadorEditor3D(canvasRef: RefObject<HTMLCanvasElement | null>, cursorFantasmaRef: RefObject<HTMLDivElement | null>, modoRenderizacao: ModoRenderizacaoEditor3D, configuracaoCameraJogo?: ConfiguracaoCameraJogo3D): boolean {
    ...
    useEffect(() => {
        if (!renderizacaoEditor3DPermiteControleCamera(modoRenderizacao)) return;

        const canvas = canvasRef.current;

        if (canvas === null) return;

        const controle = { canvas, cursorFantasma: cursorFantasmaRef, refs };

        return renderizacaoEditor3DExibeAmbienteEdicao(modoRenderizacao) ? registraEventosRenderizadorEditor3D(controle) : registraEventosCameraRenderizadorEditor3D(controle, configuracaoCameraJogo ?? { modoCamera: 'TERCEIRA_PESSOA' });
    }, [canvasRef, configuracaoCameraJogo, cursorFantasmaRef, modoRenderizacao, refs]);
