...
export function RenderItensPermissoes(permissoes: NivelPermissao[], url: string, divisor: boolean) {
    return (
        <>
            ...
            <div id={styles.recipiente_lista_acoes}>
                {permissoes.filter(nivel => nivel.condicao && nivel.itens.length > 0).map((nivel, indexPermissao, arrayFiltrada) => (
                    <React.Fragment key={indexPermissao}>
                        ...
                        {nivel.itens.map((item, index) => RenderItemBase(item, `${index}`, url))}
...
export function RenderItemBase(item: ItemMenu, key: string, url: string): JSX.Element {
    ...
    return (
        <div key={key} className={styles.recipiente_item_lista_acoes}>
            {item.link === undefined ? (
                ...
                <CustomLink href={`/minhas-paginas/${url}/${item.link}`}>
                    <h2>{item.titulo}</h2>
                </CustomLink>
            )}
            ...
            {temSubitens && (
                <div className={styles.recipiente_subitens_lista_acoes}>
                    {item.subitens!.map((sub, idx) => RenderItemBase(sub, `${key}-${idx}`, url))}
                </div>
            )}
        </div>
    );
