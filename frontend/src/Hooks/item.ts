import { criarEfeito, criarNomeCustomizado, CustoAcaoExecucao, DadosItem, Duracao, Elemento, Extremidade, Inventario, Item, NivelComponente, pluralize, PrecoExecucao, RLJ_Ficha2, TipoItem } from 'Classes/ClassesTipos/index.ts';
import { SingletonHelper } from 'Classes/classes_estaticas.ts';

import { criarCustoAcaoExecucao } from 'Hooks/custosExecucao.ts';

export const criarItem = (dadosItem: DadosItem, informacoesContextuais: { dadosPersonagem: RLJ_Ficha2, getInventario: () => Inventario, extremidades: Extremidade[], podePagarPreco: (precos: PrecoExecucao[]) => boolean, pagaPrecoExecucao: (precos: PrecoExecucao[]) => void, resumoPagamento: (precos: PrecoExecucao[]) => string[], modificarItemDoInventario: (identificadorNomePadrao: string, modificador: (item: DadosItem) => boolean) => void}): Item => {
    const { dadosPersonagem, getInventario, extremidades, podePagarPreco, pagaPrecoExecucao, resumoPagamento, modificarItemDoInventario } = informacoesContextuais;
    ...
    const item: Item = {
        ...
        get quantidadeUnidadesDesseItem(): number {
            ...
            return (!this.agrupavel || this.itemEstaEmpunhado) ? 1 : inventario.itens.filter(item => !item.itemEstaEmpunhado && item.nome.nomeExibicao === this.nome.nomeExibicao).length
        },
