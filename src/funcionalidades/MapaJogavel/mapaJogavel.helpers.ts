...
// O mapa jogável vive em DUAS convenções ao mesmo tempo (Z-up, Blender):
// - a CENA do projeto MAPA (metros, plano XY livre em torno da origem do Editor 3D; Z é a altura);
// - o MAPA LÓGICO da Partida (milímetros, x∈[0..largura], y∈[0..altura]).
// A ponte é o bbox XY da cena: o canto mínimo do bbox é a ORIGEM lógica (0,0); x lógico = X da cena; y lógico = Y da cena.
// Em ALTURA (eixo Z) não há ponte nenhuma: o grid do Editor 3D (z=0 da cena) É o chão do jogo — o que o autor pousa no grid,
...
export type DimensoesMapaJogavel = {
    ...
    // Canto mínimo do bbox XY em METROS da cena — a origem lógica (0,0) do mapa.
    origemXMetros: number;
    origemYMetros: number;
};
...
export function elementosDoMapa(cena: CenaCanonicaEditor3D): ElementoDoMapaJogavel[] {
    ...
    for (const objeto of cena.objetos) {
        ...
        elementos.push({
            ...
            nome: objeto.nome,
            // Z-up: posição lógica = centro no plano XY; largura = extensão X; profundidade = extensão Y (2ª horizontal); altura = extensão Z (o "para cima").
            posicao: { x: Math.round(((minX + maxX) / 2 - dimensoes.origemXMetros) * MILIMETROS_POR_METRO_MAPA), y: Math.round(((minY + maxY) / 2 - dimensoes.origemYMetros) * MILIMETROS_POR_METRO_MAPA) },
            ...
            profundidadeMilimetros: Math.round((maxY - minY) * MILIMETROS_POR_METRO_MAPA),
        });
    }
    return elementos;
};

// Dimensões lógicas do mapa derivadas do bbox XY da cena (Z-up; arredondadas a mm inteiro — o runtime valida inteiro).
export function dimensoesMapaDaCena(cena: CenaCanonicaEditor3D): DimensoesMapaJogavel | null {
    ...
    let maxY = -Infinity;
    for (const objeto of cena.objetos) {
        for (const [x, y] of verticesMundoDoObjeto(objeto)) {
            if (x < minX) minX = x;
            if (x > maxX) maxX = x;
            if (y < minY) minY = y;
            if (y > maxY) maxY = y;
