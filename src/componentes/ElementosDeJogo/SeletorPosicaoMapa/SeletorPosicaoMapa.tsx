...
import { celulaDoMundo, mundoX, mundoY, useReforcaRedimensionamentoCanvas } from 'Componentes/ElementosDeJogo/TelaDeJogo/cenaSalaJogo.helpers';
    ...
    return (
        <div className={styles.seletor}>
            <div className={styles.cena_mapa}>
                {cenaMapa !== null ? (
                    <Canvas shadows dpr={[1, 2]} resize={{ offsetSize: true }} camera={{ position: [0.5 + distancia * 0.62, 0.5 - distancia * 0.62, distancia * 0.8], up: [0, 0, 1], fov: 38, near: 0.1, far: distancia * 8 }}>
                        ...
                        {marcadoresContexto.map(marcador => <MarcadorContextoAssentado key={marcador.key} x={mundoX(marcador.posicao.x, larguraMilimetros)} y={mundoY(marcador.posicao.y, alturaMilimetros)} />)}
                        <MarcadorPosicaoAtiva x={mundoX(posicao.x, larguraMilimetros)} y={mundoY(posicao.y, alturaMilimetros)} />

                        <OrbitControls makeDefault enablePan enableZoom enableRotate enableDamping target={[0.5, 0.5, 0]} minDistance={1.5} maxDistance={distancia * 4} />
                    </Canvas>
                ) : (
...
function PlanoArrastoPosicao({ largura, altura, aoMudarPosicao }: { largura: number; altura: number; aoMudarPosicao: (posicao: PosicaoMapa) => void }) {
    ...
    function aplica(evento: ThreeEvent<PointerEvent>): void {
        aoMudarPosicao(celulaDoMundo(evento.point.x, evento.point.y, largura, altura));
    };
    ...
    return (
        <mesh position={[0.5, 0.5, 0.01]} onPointerDown={aoDescer} onPointerMove={aoMover} onPointerUp={aoSoltar} onPointerCancel={aoSoltar}>
...
function MarcadorContextoAssentado({ x, y }: { x: number; y: number }) {
    const alturaApoio = useAlturaApoioNoMapa(x, y);
    return (
        <mesh position={[x, y, alturaApoio + 0.12]} raycast={() => null}>
...
function MarcadorPosicaoAtiva({ x, y }: { x: number; y: number }) {
    const alturaApoio = useAlturaApoioNoMapa(x, y);
    return (
        <group position={[x, y, alturaApoio]} raycast={() => null}>
            {/* Z-up: o pino (haste + cabeça) é modelado em Y-up; orientamos o bloco para ficar em pé no eixo Z. */}
