...
export interface OpcoesDesenhoArestasEdicaoEditor3D {
    readonly alphaBase: number;
    readonly ignoraProfundidade: boolean;
};
...
const opcoesPadraoDesenhoArestasEdicaoEditor3D: OpcoesDesenhoArestasEdicaoEditor3D = { alphaBase: 0.9, ignoraProfundidade: false };
...
export function desenhaArestasSelecaoObjetoEditor3D(gl: WebGLRenderingContext, programa: ProgramaEditor3D, arestasEdicao: ArestasEdicaoRenderizadasEditor3D, matrizPerspectiva: Float32Array, matrizCamera: Float32Array, matrizObjeto: Float32Array, emModoTransformacao: boolean): void {
    const config = emModoTransformacao ? selecaoModoEditor3D : selecaoObjetoEditor3D;
    ...
    desenhaMalhaEditor3D(gl, programa, arestasEdicao.buffers, arestasEdicao.geometria, config.corBase, config.corLuz, 0.92);
...
export function desenhaArestasEdicaoEditor3D(gl: WebGLRenderingContext, programa: ProgramaEditor3D, arestasEdicao: ArestasEdicaoRenderizadasEditor3D, arestaSelecionada: ArestaSelecionadaEdicaoEditor3D | null, idObjeto: string, matrizPerspectiva: Float32Array, matrizCamera: Float32Array, matrizObjeto: Float32Array, opcoes: OpcoesDesenhoArestasEdicaoEditor3D = opcoesPadraoDesenhoArestasEdicaoEditor3D): void {
    ...
    if (opcoes.ignoraProfundidade) gl.disable(gl.DEPTH_TEST);
    gl.enable(gl.BLEND);
    gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
    gl.depthMask(false);
    gl.disable(gl.CULL_FACE);
    ...
    gl.polygonOffset(-1, -1);
    aplicaMatrizesEditor3D(gl, programa, matrizFinal, matrizObjeto);
    desenhaMalhaEditor3D(gl, programa, arestasEdicao.buffers, arestasEdicao.geometria, corBaseArestaEdicaoEditor3D, corLuzArestaEdicaoEditor3D, opcoes.alphaBase);
    ...
    gl.depthMask(true);
    gl.disable(gl.BLEND);
    if (opcoes.ignoraProfundidade) gl.enable(gl.DEPTH_TEST);
};
