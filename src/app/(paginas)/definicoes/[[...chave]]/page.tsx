...
export default function PaginaDefinicao({ params }: { params: { chave?: string[] } }) {
    return (
        <ControladorSlot pageConfig={{ comCabecalho: true, usuarioObrigatorio: false }}>
            <PaginaDefinicao_Slot params={params}/>
        </ControladorSlot>
    );
};

export async function PaginaDefinicao_Slot({ params }: { params: { chave?: string[] } }) {
