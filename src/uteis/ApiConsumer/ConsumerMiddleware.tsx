...
export async function obtemConfigNavegacao(): Promise<NavegacaoConfigRuntime> {
    return await useApi<NavegacaoConfigRuntime>({ uri: '/navegacao/config-runtime', method: 'GET' });
};
...
export async function criaMusicaConfigurada(payload: DTO__CREATE__MusicaConfigurada): Promise<boolean> {
    return await useApi<boolean>({ uri: '/musicasConfiguradas/criaMusicaConfigurada', method: 'POST', data: payload });
}

export async function atualizaMusicaConfigurada(payload: DTO__UPDATE__MusicaConfigurada): Promise<boolean> {
    return await useApi<boolean>({ uri: '/musicasConfiguradas/atualizaMusicaConfigurada', method: 'POST', data: payload });
}

export async function criaDimensaoClima(payload: DTO__CREATE__DimensaoClima): Promise<boolean> {
    return await useApi<boolean>({ uri: '/dimensoesClima/criaDimensaoClima', method: 'POST', data: payload });
}

export async function definirClimaMusica(payload: DTO__DEFINIR__ClimaMusica): Promise<boolean> {
    return await useApi<boolean>({ uri: '/musicasConfiguradas/definirClimaMusica', method: 'POST', data: payload });
}
...
// Salas de chat (admin, gate SUDO) — escrita REST da sala base do chat; estado controla o ciclo de vida (TRANCADA = ninguém escreve).
export async function criaSalaChat(payload: PAYLOAD__CriarSalaChat): Promise<{ id: number }> {
    return await useApi<{ id: number }>({ uri: '/salasChat/criaSalaChat', method: 'POST', data: payload });
}

export async function atualizaSalaChat(payload: PAYLOAD__AtualizarSalaChat): Promise<{ id: number }> {
    return await useApi<{ id: number }>({ uri: '/salasChat/atualizaSalaChat', method: 'POST', data: payload });
}

export async function defineEstadoSalaChat(payload: PAYLOAD__DefinirEstadoSalaChat): Promise<{ id: number }> {
    return await useApi<{ id: number }>({ uri: '/salasChat/defineEstadoSalaChat', method: 'POST', data: payload });
}
...
// Admin (SUDO): todos os pagamentos confirmados — carga inicial da tela em tempo real.
export async function obterPagamentosAdmin(): Promise<PagamentoAdminDto[]> {
    return await useApi<PagamentoAdminDto[]>({ uri: '/pix/admin/pagamentos', method: 'GET' });
}
