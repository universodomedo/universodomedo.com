...
export function useEditor3D(): Editor3DModelo {
    ...
    const acoes = useMemo<Editor3DAcoes>(() => ({
        ...
        iniciaBevelSelecao: () => dispatch({ tipo: 'INICIA_BEVEL_SELECAO' }),
        atualizaBevelEmEdicao: deltaLargura => dispatch({ tipo: 'ATUALIZA_BEVEL_EM_EDICAO', deltaLargura }),
        alteraSegmentosBevelEmEdicao: deltaSegmentos => dispatch({ tipo: 'ALTERA_SEGMENTOS_BEVEL_EM_EDICAO', deltaSegmentos }),
        confirmaBevelEmEdicao: () => dispatch({ tipo: 'CONFIRMA_BEVEL_EM_EDICAO' }),
        cancelaBevelEmEdicao: () => dispatch({ tipo: 'CANCELA_BEVEL_EM_EDICAO' }),
        ...
        aplicaMaterialVisualObjetoSelecionado: materialVisual => dispatch({ tipo: 'APLICA_MATERIAL_VISUAL_OBJETO_SELECIONADO', materialVisual }),
