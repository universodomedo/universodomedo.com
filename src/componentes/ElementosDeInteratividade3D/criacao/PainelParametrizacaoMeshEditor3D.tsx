...
import type { CampoVetorMalhaEditor3D, IndiceVetor3Editor3D } from '../editor/editor3D.tipos';
...
export function PainelParametrizacaoMeshEditor3D() {
    ...
    useEffect(() => {
        function confirmaCriacaoAoClicarFora(event: MouseEvent): void {
            const painel = painelRef.current;

            if (malha === null || painel === null) return;
            if (!(event.target instanceof Node)) return;
            if (painel.contains(event.target)) return;
            if (event.target instanceof Element && event.target.closest('[data-editor3d-menu-criacao="true"]') !== null) return;

            acoes.confirmaMalhaEmCriacao();
            event.preventDefault();
            event.stopPropagation();
        };

        document.addEventListener('mousedown', confirmaCriacaoAoClicarFora, true);

        return () => document.removeEventListener('mousedown', confirmaCriacaoAoClicarFora, true);
    }, [malha, acoes]);
    ...
    return (
        <div ref={painelRef} className={styles.painelParametrizacaoMesh} data-editor3d-painel-parametrizacao-mesh="true">
            <header className={styles.cabecalhoParametrizacaoMesh}>
                <span>Add {definicao.nome}</span>
            ...
            <footer className={styles.rodapeParametrizacaoMesh}>Clique fora para confirmar a criação.</footer>
        </div>
    );
};